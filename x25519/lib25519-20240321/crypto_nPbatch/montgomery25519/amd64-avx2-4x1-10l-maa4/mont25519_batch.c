...
int crypto_nP_4x1(unsigned char q[4][CRYPTO_BYTES], const unsigned char n[4][CRYPTO_BYTES], const unsigned char p[4][CRYPTO_BYTES]) {
	...
	crypto_uint64_vec4x1 r[10] = {{0}};
	crypto_uint64_vec4x1 t[20] = {{0}};
	...
	for (i=0;i<4;++i) {
		...
		fe25519_unpack(&c,(const unsigned char *)(p+i));
	  	fe25519_to_10l(a+i,&c);
	}
	
	for (i=0;i<10;++i) for (j=0;j<4;++j) r[i][j] = a[j].l[i];
	...
	for (j=0;j<4;++j) {
		...
		for (i=0;i<10;++i) {
			...
			a[j+0].l[i] = t[i+0*10][j]; 
			a[j+4].l[i] = t[i+1*10][j];
		}
		fe25519_from_10l(&b[j+0],&a[j+0]);
		fe25519_from_10l(&b[j+4],&a[j+4]);
	}
